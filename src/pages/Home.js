const Home = (params, el) => {
  ...
  afterPageLoad(async () => {
    ...
    // Staggered entrance for post cards (home page only)
    ...
    const attachPostEvents = () => {
      // Save buttons
      container.querySelectorAll("[data-saveid]").forEach((button) => {
        button.addEventListener("click", (e) => {
          ...
          button.classList.toggle("bg-white", saved);
          button.classList.toggle("text-black", saved);
          button.classList.toggle("bg-[#2c2c2e]", !saved);
          button.classList.toggle("text-white/65", !saved);
      ...
      // Article links
      container.querySelectorAll("[data-url]").forEach((link) => {
